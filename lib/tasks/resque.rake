...
namespace :resque do
  ...
  task :start_daemons do
    ...
    workers_config(ENV['RESQUE_HOST']).each do |worker, config|
  ...
  def self.workers_config(queue_suffix=nil)
    resque_config_file = queue_suffix ? "config/resque_workers_#{queue_suffix}.yml" : "config/resque_workers.yml"
    YAML.load(File.open(ENV['WORKER_YML'] || resque_config_file))
  end
